...
int CreatList(ASSORT **phead)
{
    ...
    {
        printf("自然灾害类别信息数据文件打开失败！\n");
        return re;
    }
    printf("自然灾害类别信息数据文件打开成功！\n");

    /*从数据文件中读自然灾害类别信息数据，存入以后进先出方式建立的主链中*/
    while (!feof(pFile))
    {
        pAssort = (ASSORT*)malloc(sizeof(ASSORT));
        fread (pAssort,sizeof (ASSORT),1,pFile);
        pAssort->enext = NULL;
        pAssort->next = hd;
        hd = pAssort;
    }
    ...
    if(hd == NULL)
    {
        printf("自然灾害类别信息数据文件加载失败！\n");
        return re;
    }
    printf("自然灾害类别信息数据文件加载成功！\n");
    *phead = hd;
    re += 4;
    if ((pFile = fopen(gp_event_info_filename,"rb")) == NULL)
    {
        printf("自然灾害事件基本信息数据加载失败！\n");
        return re;
    }
    printf("自然灾害事件基本信息数据加载成功！\n");
    re += 8;

    /*从数据文件读取自然灾害事件基本信息存入主链对应结点的自然灾害事件基本信息支链中*/
    while (!feof(pFile))
    {
        /*创建结点，存放从数据文件中读出的自然灾害事件基本信息*/
        pEvent = (EVENT *)malloc(sizeof (EVENT));
        fread(pEvent,sizeof (EVENT),1,pFile);
        pEvent->dnext = NULL;

        /*在主链上查找该自然灾害类别所对应的主链结点*/
        pAssort = hd;
        while (pAssort != NULL && strcmp(pAssort->name,pEvent->name) != 0)
        {
            pAssort = pAssort->next;
        }

        /*如果找到，则将结点以后进先出方式插入自然灾害事件基本信息支链*/
        if (pAssort != NULL)
        {
            pEvent->next = pAssort->enext;
            pAssort->enext = pEvent;
        }
        else    /*如果未找到，释放所创建结点的空间*/
        {
            free(pEvent);
        }
    }
    ...
    /*从数据文件中读取捐款信息数据，存入自然灾害事件基本信息支链对应结点的捐款信息支链中*/
    while (!feof(pFile))
    {
        /*创建结点，存放从数据文件中读取的捐款信息*/
        pDonate = (DONATE *)malloc(sizeof (DONATE));
        fread(pDonate,sizeof (DONATE),1,pFile);

        /*查找事件基本信息支链上对应的事件基本信息结点*/
        pAssort = hd;
        find = 0;
        ...
        if (find) /*如果找到，则将结点以后进先出方式插入捐款信息支链中*/
        {
...
BOOL ShowAll(void)
{
    ...
    for(pAssort=gp_head->next;pAssort!=NULL;pAssort=pAssort->next)
    {
        if(pAssort->type) /*排除无效的节点*/
        {
...
BOOL JudgeEventItem2(EVENT *pEvent,char *pcond1,char *pcond2,char *pcond3)
{
    ...
    time1=atoi(pcond1);
    time2=atoi(pcond2);
    Time=atoi(pEvent->time);
    if((Time>=time1&&Time<=time2)&&(strcmp(pEvent->name,pcond3)==0))
    {
        bRet=TRUE;
    }
    else
...
BOOL MaintainAssort(void)
{
    ...
    char *pCh4[] = {"请输入：  原灾害类别     ","请输入：灾害类别和名称 ","开始输入","取消"};
    int iHot = 4;/*热键所在行数*/
...
BOOL MaintainDonate(void)
{
    ...
    char *pCh4[] = {"请输入：  原事件编号和捐款人姓名","请输入更改信息：捐款时间 类别 捐款者姓名 捐款数目                                 ","                                                ","开始输入","取消"};
        ...
        else
        {
            if(iHot == 3)
            {
                ...
                if(DealInput(&areas, &iHot) == 13 && iHot == 1)
                {
                    ...
                    char dname[20];
                    scanf("%s %s %s %c %s %f",pDonate2->num,dname,pDonate2->time,&(pDonate2->type),pDonate2->dname,&(pDonate2->money));
                    lp.bVisible = FALSE;
                    SetConsoleCursorInfo(gh_std_out,plp);
                    PopOff();
                    if(ModifDonate(gp_head,pDonate2->num,dname,pDonate2))
                    {
                        set_text_success();
                    }
                    else
...
BOOL QueryDonate(void)
{
    ...
    char *pCh2[] = {"请输入：编号和捐款者名称子串","开始输入","取消"};
...
BOOL StatType(void)
{
    ...
    DONATE *pDonate;
    int i,j,mid_int;
    int happen[30],dead[30],collapse[30];
    float loss[30],mid_float;
    CONSOLE_CURSOR_INFO lp,*plp = &lp;
    lp.bVisible = TRUE;
    lp.dwSize = 1;
    SetConsoleCursorPosition(gh_std_out,(COORD)
    ...
    for(pAssort=gp_head,i=0;pAssort!=NULL;pAssort=pAssort->next,i++)
    {
        for(pEvent=pAssort->enext;pEvent!=NULL;pEvent=pEvent->next)
        {
            happen[i]++;
            dead[i]+=pEvent->dead;
            collapse[i]+=pEvent->collapse;
            loss[i]+=pEvent->loss;
        }
    }
    for(j=0;j<=i;j++)
    {
        if(happen[j]<happen[j+1])
        {
            mid_int=happen[j];
            happen[j]=happen[j+1];
            happen[j+1]=mid_int;
            ...
            mid_int=collapse[j];
            collapse[j]=collapse[j+1];
            collapse[j+1]=mid_int;
    ...
    for(j=0;j<=i;j++)
    {
        if(happen[j]!=0)
        {
            printf("发生次数：%d\t",happen[j]);
            printf("死亡总人数：%d\n",dead[j]);
            printf("房屋倒塌总数：%d\t",collapse[j]);
            printf("直接经济损失总额：%f亿元\n",loss[j]);
            printf("*********************************************************");
            printf("\n");
        }
